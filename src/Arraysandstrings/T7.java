 ...
 * @Description: 机器人在一个无限大小的 XY 网格平面上行走，从点 (0, 0) 处开始出发，面向北方。该机器人可以接收以下三种类型的命令 commands ：
 *
 * -2 ：向左转 90 度
 * -1 ：向右转 90 度
 * 1 <= x <= 9 ：向前移动 x 个单位长度
        ...
        for (int i = 0; i < commands.length; i++) {
            if (commands[i] == -1) {
                ...
                if (fangxiang == 1 || fangxiang == -1) {
                    ...
                    for (int p = 0; p < obstacles.length; p++) {
                        if (obstacles[p][0] == x && obstacles[p][1] > y && y + commands[i] >= obstacles[p][1]) {
                            y = obstacles[p][1] - 1;
                            System.out.println("被 y=" + obstacles[p][1] + "挡住");
                            zjh = 1;
                            break;
                        }
                    }
                    if (zjh == 0) {
                        y += commands[i];
                        System.out.println("向北走" + commands[i]);
                    }
                ...
                if (fangxiang == 2 || fangxiang == -2) {
                    int zjh = 0;
                    for (int p = 0; p < obstacles.length; p++) {
                        if (obstacles[p][1] == y && obstacles[p][0] > x && x + commands[i] >= obstacles[p][0]) {
                            x = obstacles[p][0] - 1;
                            System.out.println("被 x=" + obstacles[p][0] + "挡住");
                            zjh = 1;
                            break;
                        }
                    }
                    if (zjh == 0) {
                        x += commands[i];
                        System.out.println("向东走" + commands[i]);
                    }
                    System.out.println(x + ",," + y);
                    if (Math.abs(x) > xmax) {
                        xmax = Math.abs(x);
                ...
                if (fangxiang == 3 || fangxiang == -3) {
                    ...
                    for (int p = 0; p < obstacles.length; p++) {
                        if (obstacles[p][0] == x && obstacles[p][1] < y && y - commands[i] <= obstacles[p][1]) {
                            y = obstacles[p][1] + 1;
                            System.out.println("被x="+p+" y=" + obstacles[p][1] + "挡住");
                            zjh = 1;
                            break;
                        }
                    }
                    if (zjh == 0) {
                        y -= commands[i];
                        System.out.println("向南走" + commands[i]);
                    }
                    System.out.println(x + ",," + y);
                    if (Math.abs(x) > xmax) {
                        xmax = Math.abs(x);
                ...
                if (fangxiang == 0) {
                    ...
                    if (zjh == 0) {
                        x -= commands[i];
                        System.out.println("向西走" + commands[i]);
                    }
                    System.out.println(x + ",," + y);
                    if (Math.abs(x) > xmax) {
                        xmax = Math.abs(x);
